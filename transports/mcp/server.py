...
Usage:
    python -m transports.mcp.server

Or as a script:
    python transports/mcp/server.py
"""
...
class MCPServer:
    ...
    def _handle_open_tensions(self, args):
        return self.engine._list_tensions()
    ...
    def _handle_propose_thread(self, args):
        pid = self.api.propose_thread(
            proposer=args.get("proposer", ""),
            tension_id=args["tension_id"],
            question=args["question"],
            reasoning_mode=ReasoningMode(args.get("reasoning_mode", "forward")),
            rationale=args.get("rationale", ""),
        )
        return {"proposal_id": pid}
            ...
            if method == "initialize":
                self._respond({
                    ...
                    "result": {
                        ...
                        "serverInfo": {"name": "superretometer", "version": "0.1.0"},
                        "capabilities": {
